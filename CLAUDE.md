...
- **Next concrete step:** build out the coming-soon units for real — phase 3 ("Steady rhythm") is the big one: falling-notes/rhythm-lane UI, metronome-synced backing track, simple public-domain tunes, timing/finger-sequence scoring. The lesson framework (`ui.tsx` + `curriculum.tsx`) is now in place to hang richer content and future interactive drills on.
